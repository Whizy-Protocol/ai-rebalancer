...
@pytest.fixture
def knowledge_agent():
    """Fixture to create KnowledgeAgent instance with URL from environment"""
...
async def test_knowledge_agent_create_retriever(knowledge_agent):
    ...
    with (
        patch("src.agent.OpenAIEmbeddings") as mock_embeddings,
        patch("src.agent.FAISS.from_documents") as mock_faiss,
    ):
        mock_retriever = Mock()
...
async def test_knowledge_agent_process_query_with_risk(knowledge_agent):
    ...
    knowledge_agent.risk_prompts = [{"risk": "low", "prompt": "Test prompt for low risk users"}]

    with (
        patch.object(knowledge_agent, "initialize", return_value=None),
        patch.object(knowledge_agent, "agent_executor") as mock_executor,
    ):
        mock_message = Mock()
        ...
        response = await knowledge_agent.process_query(
            query="What's the best protocol?", risk_level="low"
        )
...
async def test_knowledge_agent_temperature_setting(knowledge_agent):
    ...
    with (
        patch("src.agent.ChatOpenAI") as mock_chatgpt,
        patch("src.agent.RetrievalQA.from_chain_type"),
...
async def test_risk_classifier_temperature_setting():
    ...
    with patch("src.agent.ChatOpenAI") as mock_chatgpt, patch("src.agent.create_react_agent"):
...
async def test_knowledge_agent_handles_empty_knowledge_data(knowledge_agent):
    ...
    with (
        patch("src.agent.OpenAIEmbeddings"),
        patch("src.agent.FAISS.from_documents") as mock_faiss,
    ):
        mock_retriever = Mock()
        mock_faiss.return_value.as_retriever.return_value = mock_retriever
...
async def test_risk_update_in_wallet_file():
    ...
    with patch("builtins.open", create=True) as mock_open:
        # First call is for reading, second is for writing
        ...
        # Verify the data structure was updated correctly
        written_data = mock_write_file.write.call_args[0][0]
        updated_data = orjson.loads(written_data)
        assert updated_data[0]["risk_profile"] == "medium"
